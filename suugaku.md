...
そのため、2次関数のグラフの$y=ax^2$のグラフのx,yをx軸方向にp、y軸方向にq平行移動したグラフの点(x, y)を、元のグラフ$y=ax^2$の座標に対応させて表現している。  
そうすることで、具体的にわからないy=g(x)上の、$y=ax^2$のグラフのx,yをx軸方向にp、y軸方向にq平行移動した(x,y)という点を$y=ax^2$のグラフ上の点に対応させて表現している。

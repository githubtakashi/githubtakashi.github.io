...
結局は処理待ちのタスク数を表している。これの平均の処理待ち時間は、

平均処理待ち時間をEとすると、E=ρ / (1-ρ)となる。

待ち行列での利用率ρ、つまりタスクのたまり具合であるトラフィック密度は0 < ρ < 1 が前提で、ρ=1とは到着するタスクの数と  
処理できるタスクの数が等しく、ρが1に近づくほど待ち行列の処理能力が高いことを意味している。

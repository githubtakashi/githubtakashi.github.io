...
"異なるn個のものから異なるr個を選んで１列に並べる並べ方" に順列の計算方法を使う。  

順列の公式：  
${}nPr = n(n-1)(n-2)... (n-r+1)= \frac{n!}{(n-r)!}$

例えば、異なる５個のa,b,c,d,eから３個選んで並べる場合

(n-r+1)の部分は、5-3+1=2+1=3となる。つまり、(n-r+1)で5x4x3の３個選ぶことができる。  
...
順列だと5個から2個選んで順番に並べるので、5x4=20通りで組み合わせの倍となる。順列だと順番に並べるので、{ab}と{ba}. 
異なるけど、組み合わせで考えると同じものなので、2個の並べ方で1個の組み合わせとなる。

10個の組み合わせについてそれぞれ2個の並べ方がある。  

abのときab, ba. 
acのときac, ca. 
...

なので、20÷2=10個の組み合わせとなる。順列は並べるので並べ方を考えるけど、組み合わせは並べ方は考えないため。

組み合わせの公式：  
${}{n}C{r} = \frac{{}{n}P{r}}{r!} = \frac{n(n-1)...(n-r+1)}{r(r-1)...3 \cdot 2 \cdot 1} = \frac{n!}{r!(n-r)!}$
...
上記の式より${}{5}C{3}　= \frac{{}{5}P{3}}{3!} = \frac{5 \cdot 4 \cdot 3}{3 \cdot 2 \cdot 1} = 10$

同様にして考えると、n個からr個とる組み合わせの数は、

${}{n}C{r} \cdot r! = {}{n}P{r}$になり、これより

${}{n}C{r} = \frac{nPr}{r!} = \frac{n(n-1)...(n-r+1)}{r(r-1)...3 \cdot 2 \cdot 1}$が成り立つ。

また、${}{n}P{r} = \frac{n!}{(n-r)!}$より、

${}{n}C{r} = \frac{{}nPr}{r!} = \frac{n!}{(n-r)!} \cdot \frac{1}{r!} = \frac{n!}{r!(n-r)!}$

だから${}{n}C{r} = \frac{n!}{r!(n-r)!}$

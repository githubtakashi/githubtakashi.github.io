...
以上から、M/M/1モデルは、1つの窓口にランダムにタスクが到着し、窓口が処理するタスクの処理時間は指数分布の待ち行列を表している。

### M/M/1モデルの最初のMは窓口に到着するタスクの状態を表している
  
タスクが窓口に到着する様子(10人集まる確率が80%、1000人集まる確率が0.001%など)を表している。

Mの部分に入る記号として、Mの他に、性質によって9種類ほどある。  
応用情報の勉強ではM/M/1モデルしか出題されないぽいので、下記の"M"という記号の定義だけ覚えておけば大丈夫。
...
発生する間隔については2年連続で発生し、その後ずっと発生せず10年後に1回発生する。みたいに、いつ起こるかについては  
ランダムなイベントを表す。  
...
連続確率変数では、x軸の2点間の範囲のグラフ下側の範囲の面積が確率に対応しているため、x軸のある1点の確率を求めようとしても、  
面積は0となり確率が0になる。  
...
1時間あたり50件到着するというような感じに、ある時間当たりに窓口に到着するタスクの件数を表す。  
...
となり、計算結果としてのρは到着するタスクの処理のでき具合を表している。
...
もしρが1以上だと、到着するタスクの数よりも処理するタスクの数が少ないことになり、到着するタスクの数に対して窓口の処理が追いついていないことを意味する。  
...
タスク1件当たりの処理時間は、タスク1件を平均処理率μで割ったものになる。  
...
1件のタスク処理にかかる時間にタスクの到着具合を掛けることで、利用率が計算できる感じ。
...
ρが0に近づくほど待ち行列の処理能力が高く、1に近づくほど処理能力が低いことを意味しているので、1は窓口の能力の最大値を表している。
...
E=0.01/0.99=0.01...となり、処理中のタスクも含めて停滞しているタスク数の数は、0.01...で1個以下なので、限りなく少なくなることが分かる。

...
代表的な待ち行列の理論はM/M/1型で、この表記方法をケンドールの記号と呼ぶ。

ケンドールの記号とは、待ち行列の特徴を表すための記号のこと。

待ち行列に存在するキューの状態・特徴を表現するために"A/B/C"という形式で、3つの要因でキューの特徴を表現する。

M/M/1の最初のMは処理窓口に到着するタスクの回数(頻度)に着目した分布を表していて、  
Mは大まかに分類するとランダムな分布である。  

2番目のMは、処理窓口に到着したタスクの処理時間の分布を表している。時間に着目した分布を表している。  
2番目のMは指数分布であることを表している。

3番目の1は、処理できる窓口が1つであることを表している。

以上から、M/M/1モデルは、1つの窓口にランダムにタスクが到着し、窓口の処理時間は指数分布である待ち行列を表している。

### Aの部分はタスクが窓口にどんな分布で到着するかを表している

処理したいタスクがキューにたまっていくとすると、A/B/CのAはタスクの到着の過程と到着の分布を示している。  
キューへの到着方法の性質(ランダムな時間間隔で集まるなど)、分布(10人集まる確率が80%、1000人集まる確率が0.001%など)を表している。

Aの部分に入る記号として、性質によって9種類ほど区分し定義している。  
応用情報の勉強ではM/M/1モデルしか出題されないぽいので、下記の"M"という記号の定義だけ覚えておく。
...
ポアソン過程とは、例えば、地震Aが発生すると次同じ地震が発生するまでの平均時間が分かっているけど、発生する間隔については、  
2年連続で発生したり、10年後に1回発生するなど、いつ起こるかについてはランダム性を持ったイベントをモデル化した概念。  

平均時間と聞くと等間隔のように錯覚しがちなので注意する。

例)Aという地震が起こる平均間隔の時間は1年だと分かっている。Aが毎年1回発生し3回連続して起こった。その後、5年間何も発生せず、2年連続してAが発生。  
のようなランダムなイメージ。

ポアソン分布は、確率変数の値をある関数で表し、それをグラフにしたもの。縦軸が確率、横軸が回数や時間を表している。  
特定の確率と回数の分布が見た目で分かるようになっている。コールセンターへの電話が5回かかってくる確率が80%、1000回かかってくる確率は0.001%など。
...
指数分布とポアソン分布は縦軸はどちらも確率を表している。指数分布とポアソン分布は横軸が着目するものが異なる。  
...
ｘ人来店がある確率を求めたい場合、ポアソン分布の計算式で求めることができる。グラフからも大体の確率を読み取ることができる。    
以上から、ポアソン分布は単位時間内に事象が起こる回数についての確率の分布となる。  

1時間という単位時間あたりに平均100人来客が来る場合、その事象の発生間隔が1時間じゃなく例えば10分単位など  
ある時間での確率に着目し、分布を表したものが指数分布。  

以上から、ある事象の回数に着目する場合はポアソン分布、期間に着目する場合は指数分布という風に区別する。
...
どれかの目が出る確率は、1/6 * "目の数が無限" = "無限"  
となり、どれかの目が出る可能性が1を超え、無限となりおかしなことになる。  
これは、連続確率変数を扱うときは、通常の確率の考え方が使い物にならないことを意味している。

そこで登場するのが、確率密度という考え方。

確率密度では、確率の合計が１になるように定義する。縦ｘ横の面積が1である面積に確率を対応させる。横軸が確率変数、縦軸が確率。  
連続確率変数では、x軸の2点間の範囲の面積が確率に対応しているため、x軸のある1点の確率を求めようとしても、面積は0となり確率が0になる。  
そのため、連続確率変数はx軸のある1点での確率は求めることができないことに注意する。

サイコロの1,2,3,4,5,6のようにとびとびの値である離散確率変数の頻度をヒストグラムにして、離散の値を連続にすると、ヒストグラムが曲線になる。  
...
時間あたり50件到着するというような感じに、時間当たりに到着するタスクの件数を表す。  
平均到着率は、一定時間の間に到着してくるタスクの数を表している。例えば2時間の間に100件到着など。

窓口へのタスクの到着の件数について扱うのが平均到着率。
...
利用率は、タスクを処理する窓口がタスクを処理している最中である確率を表している。
...
窓口でタスクを処理するので、タスク1件を処理するのにどのくらい時間がかかるかについても考慮する必要がある。

タスク1件当たりの処理時間は、タスク1件をタスクの処理能力(平均処理率μ)で割ったものになる。  
(タスク1件当たりの処理時間) = (タスク1件) / (平均処理率μ) = 1 / μ = 件 / (件/時間) = (時間) が求められる。  

例えば1時間という単位時間あたりに、100件のタスクを処理できるとすると、平均処理率μ = 100件/1時間 = 100(件/時間)。

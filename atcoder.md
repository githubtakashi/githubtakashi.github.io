...
以上から、is_prime関数でN=1,2,または3以上の選別を行い、
  
引数がN≧3である場合はhas_divisor関数を呼び出して素数であるかを判定する。

has_divisor関数では再帰ステップでiをインクリメントしつつNをiで割っていき、

割り切れた時点で素数でないと確定する。すなわち約数が存在することを意味するtrueを返す。
  
再帰ステップがここで終わるのでこれがベースケースになる。
  
割り切れずi==Nとなった場合は、ずっと割り切れる数が存在しなかったことを意味しているので、

Nは素数であることが確定する。すなわち約数が存在しないことを意味するfalseを返す。
  
これも再帰ステップがここで終わるのでベースケースとなる。

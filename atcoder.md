...
NはN自身では割り切れてしまうので、iからN-1の範囲にNの約数が存在するかを判定する。
  
例えばN = 4だったら、i～3の範囲で約数が存在するかを判定する。
...
そもそも合わないので1は渡さないようにする。
  
2は素数。N=2を渡すとi=N-1=1となり1でしか割れないので関数の意味合い的に適切ではない。
  
なので引数はN≧3を渡す。
  
返り値は、iからN-1の範囲に約数が存在したら素数ではなく、存在しなかったら素数と判定するのでboolとする。
  
処理内容は、iからN-1の範囲に約数が存在するかどうかを判定する。

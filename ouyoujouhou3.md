...
[JIS X 0160および共通フレーム](https://ssaits.jp/promapedia/concepts/jis-x-0160.html)2013においては、下記のような工程の表現と流れになる。表現は違って見えるが、上記の流れと  
同じことを言っている。
...
- 共通フレームとは  
システム開発において使われるガイドライン。関係者の認識を揃えて足並みを揃えるためにある。  
構想から開発、運用、保守、廃棄までのライフサイクルを通じて必要な作業項目、役割等を包括的に規定した共通の枠組みのこと。
...
オブジェクト指向でソフトウェア設計するときは、ウォーターフォールモデルのように後戻りできないという問題が  
...
モジュール強度とモジュール結合度についての問題が頻出しているので、しっかりと頭に入れておく。
...
モジュールの改修が入ったときに、影響範囲がどこまで広がるかを考えると、モジュール強度が強く、結合度が弱い  
モジュール設計にしたほうがよいことが分かる。
...
プロジェクトが提供する成果物及びそれを作り出すために必要な作業、つまり、プロジェクト全体のことをまとめてプロジェクトスコープと呼ぶ。  
...
プロジェクトスコープマネジメントでは、スコープ計画・スコープ定義ができたら、WBS(Work Breakdown Structure)を作成する。  
WBSによりプロジェクトの見積もりやプロジェクトの作業範囲を明確にすることで、作業スケジュールが作りやすくなる。

WBSを作成しプロジェクトが進行しだしたら、スコープ検証とスコープコントロールを実施し、  
プロジェクトを評価し見直しする。これによって、プロジェクト進行中の軌道修正、是正など適切に実施できる。  

以上のプロジェクトスコープマネジメントは、PDCAサイクルと同等の手順となっているので、  
プロジェクトスコープマネジメントとはPDCAであるともいえる。プロジェクトスコープマネジメントにおいては、  
PDCAのPに相当するのがスコープ計画、Dに相当するのがスコープ定義及びWBS、Cに相当するのがスコープ検証、Aに相当するのがスコープコントロールとなる。
...
- PV(Planned Value：出来高計画値)  
 ある時点における費用総額の予測値。なぜある時点の予測値が分かるかというと、PVは線図にするとカーブを描いた線になっていて、  
 予算の計画線なので、線上のある任意の時点の点は、計画通りに進んだ場合どのくらいの累計で予算がかかるかを表しているモデル線図になっているから。

- BAC(Budget At Completion：完成時総予算)  
 プロジェクト完了時点におけるPV。すなわち予算の合計金額のこと。
 
- EV(Earned Value：出来高実績値)  
 作業の実績値の累計。完了済の作業に対する予算。EVを線図にすることで、PVとの差が可視化され計画に対しての進捗が分かる。  
 ここで注意なのが、完了済の作業に対する"予算"なので、実際にかかったコストではないことに注意。
 
- AC(Actual Cost：コスト実績値)  
 実際にかかった費用の累計。ACも線図にすることで、PV線図やEV線図と比較でき、ある任意の時点においてコストが超過していないかなどを比較できる。
 
- EAC(Estimate At Completion：完成時総コスト見積り)  
 完成するまでの実際の総コストの予測値。  
 EACの求め方：EAC=AC+(BAC-EV)/CPI  
 
 - BAC-EV  
  完成までにかかる計画上の総予算であるBACからある時点での完了済の作業に対する予算を引いたものなので、  
  ある時点での残りに使用可能な予算を表している。
 
 - CPI  
  Cost Performance Index：コスト効率指数と呼ばれるもので、投入したコストに対してどの程度の成果を上げているかを表す指数。  
  CPI=EV/ACで求まる。ある時点でのEVに対してACが小さい場合は、作業の進みに対して、実際のコストが低コストであることを表しているので、  
  コスト効率がよいことを意味している。逆に、ある時点でのEVに対してACが大きい場合は、作業の進みに対して、実際のコストが高コストである  
  ことを表しているので、コスト効率がわるいことを意味している。  
  CPIが1より大きい場合は、EV>ACであり作業の進みに対して実コストが想定よりも低コストで済んでおり、コスト効率がよい。  
  逆に、CPIが1より小さい場合は、EV<ACであり想定よりも高コストになっていることを表している。
  
 - (BAC-EV)/CPI  
  BAC-EVはある時点での使える残りの予算を表しているので、これをCPIで割ると、残りの作業に対するの実コストの予測値が求まる。  
  例えば、残りの予算が100万円とすると、CPIが1の場合は、コスト効率は予定通りなので、100/1=100万円が残りの作業にかかる実コストの予測値。  
  CPIが2とすると、100/2=50万円が残りの作業にかかる実コストの予測値で、予算よりも低コストで済むことが予測される。  
  CPIが0.5とすると、100/0.5=150万円が残りの作業にかかる実コストの予測値で、予算よりも高コストでこのままでは予算を50万円超過  
  してしまうことを意味している。
  
 - AC+(BAC-EV)/CPI  
  残りの作業にかかる実コストの予測値である(BAC-EV)/CPIにこれまでにかかった実コストACを足したものなので、  
  これが完成するまでの実総コストの予測値となる。 
  
<br />
  
- SPI(Schedule Performance Index：スケジュール効率指数)  
 予定に対する進捗度を表す指標で、EVとPVを使って算出する。SPI=EV/PV  
 EV>PVでSPIが1より大きいと作業が予定よりも進んでいる、つまり作業効率がよいことを意味している。  
 逆に、EV<PVでSPIが1より小さいと作業が予定よりも進んでいない、つまり作業効率がわるいことを意味している。
...
将来価値は、100ｘ(1+0.10)^3という計算式で求めることができる。
...
このため、利益を出すためには、投資額を100万円未満に抑えないと利益がでないことがわかる。
見かけ上は、100万円投資したとすると100万円が133.1万円に増えているけど、100万円未満で133.1万円を得る権利を入手できないと、
利益が出ないと考えることが正味現在価値の考え方。直観に反して見えるけど。

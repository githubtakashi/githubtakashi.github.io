...
- EAC(Estimate At Completion：完成時総コスト見積り)  
 完成するまでの実際の総コストの予測値。完成していないある時点から、実際の総コストを予測するために計算する。  
 EACの求め方：EAC=AC+(BAC-EV)/CPI  
 
 - BAC-EV  
  完成までにかかる計画上の総予算であるBAC(完成時のPV値)からある時点での完了済の作業に対する予算を引いたものなので、  
  ある時点での、残り使える予算を表している。
 ...
 - (BAC-EV)/CPI  
  ...
  現在のコスト効率であるCPIがこのまま続く前提として計算する。  
  残りの予算(BAC-EV)が100万円とすると、CPIが1の場合は、コスト効率は予定通りなので、100/1=100万円が残りの作業にかかる実コストの予測値。  

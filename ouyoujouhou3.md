...
攻撃内容がパケットのデータ部分に書かれているのに対し、ファイアーウォールのパケットフィルタリングはデータの内容でフィルタリングできないため。  
攻撃が書かれたパケットがファイアーウォールを通過しwebサーバーのアプリケーションプログラムに届くことが可能になる。  
そのため、WAFで対応することが必要になる。

<br />

### パケットフィルタリング

パケットのヘッダの内容をチェックして通過させるパケットと遮断させるパケットを識別する仕組み。  

...
- EAC(Estimate At Completion：完成時総コスト見積り)  
 ...
 - BAC-EV  
  完成までにかかる計画上の総予算であるBACからある時点での完了済の作業に対する予算を引いたものなので、ある時点での残りの予算を  
 表している。
 ...
 - CPI  
  ...
  ことを表しているので、コスト効率がわるいことを意味している。CPIが1より大きい場合は、想定よりも低コストで済んでおり、コスト効率がよい。  
  逆に、CPIが1より小さい場合は、想定よりも高コストになっていることを表している。
  
 - (BAC-EV)/CPI  
  BAC-EVはある時点での使える残りの予算を表しているので、これをCPIで割ったものは、残りの作業に対するの実コストの予測値が求まる。  
  例えば、残りの予算100万円とすると、CPIが1の場合は、コスト効率は予定通りなので、100/1=100万円が残りの作業にかかる実コストの予測値。  
  CPIが2とすると、100/2=50万円が残りの作業にかかる実コストの予測値で、想定よりも低コストで済むことが予測される。  
  CPIが0.5とすると、100/0.5=150万円が残りの作業にかかる実コストの予測値で、想定よりも高コストになり、このままでは予算を50万円超過  
  してしまうことを意味している。
  
 - AC+(BAC-EV)/CPI  
  残りの作業にかかる実コストの予測値である(BAC-EV)/CPIにこれまでにかかった実コストACを足したものなので、これが完成するまでの実総コストの予測値となる。 

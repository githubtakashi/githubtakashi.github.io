...
プロジェクトが提供する成果物及びそれを作り出すために必要な作業のこと、要はプロジェクト全体のことをまとめてプロジェクトスコープと呼ぶ。  
...
プロジェクトスコープマネジメントでは、スコープ計画・スコープ定義ができたら、WBS(Work Breakdown Structure)を作成し、  
プロジェクトの見積もりやプロジェクトの作業範囲を明確にすることで、作業スケジュールが作りやすくなる。

WBSを作成しプロジェクトが進行しだしたら、スコープ検証とスコープコントロールを実施することで、  
プロジェクトを評価し見直しする。これによって、プロジェクト進行の軌道修正などが適切に実施できる。  

以上のプロジェクトスコープマネジメントは、PDCAサイクルと同等の手順となっているので、  
プロジェクトスコープマネジメントとはPDCAであるともいえる。プロジェクトスコープマネジメントにおいては、  
PDCAのPに該当するのがスコープ計画、Dに該当するのがスコープ定義及びWBS、Cに該当するのがスコープ検証、Aに該当するのがスコープコントロールとなる。

...
あるプログラム言語のソースファイルの中にIDL特有の書き方でインターフェースの情報を記述することで定義する形式で書く。  
"[]"の中に記述することが特徴。
...
デュアルインターフェースとは、c++とc++以外のスクリプト言語のどちらからもCOMサーバーを利用できるよう対応した  
インターフェースのこと。

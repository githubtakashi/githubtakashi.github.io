...
ベアラ信号とは通信回線に圧縮しないで素の状態で通す最小単位のビットことでこれをフレームと呼ぶ。  

ベアラ信号を送る速度と、通常の通信速度は別物。

これに対しデータのみの通信速度は、データを圧縮することで回線を通すデータの容量を大きくしている。  
これに対し通信回線を通るデータを圧縮してない素の状態で通すことをベアラと呼ぶ。  

データ部分は圧縮されるのに対し、これ以上圧縮しようがないベアラ信号の方は最小限のデータ量となっているので、  
ベアラ速度の方が圧縮された通常のデータ部分よりも通信速度が速い。

...
AB+の場合は、+演算子の直前2つの変数はABなので、普通の式に戻すとA+Bとなる。  
AB+CD...のように、演算子の右側に変数がある場合は、演算子の直近の前後の変数に対して計算をする。  
よって、AB+CD...のAB+CDという部分について逆ポーランド記法を元の式に戻すには、  
B+Cを計算し、これをPなど、1つの文字に置き換える。以降、最後まで同じことを繰り返し、  
文字を元の文字に戻すと、元の計算式の表現ができあがる。

...
[ネットワーク層について分かりやすい記事](https://medium-company.com/%e3%83%8d%e3%83%83%e3%83%88%e3%83%af%e3%83%bc%e3%82%af%e5%b1%a4/)を参照する。
...
インターネット上で通信する際の主要な通信プロトコル。IPアドレスについて定義している(IPv4、IPv6)。

- ICMP(Internet Control Message Protocol)
主要なプロトコルのIPを補助する役目を持った通信プロトコル。IPはコネクションレス型で、一方的に送信する方式であるが、  
その際に発生したエラーなどをICMPし検知し、報告するためのプロトコル。

- IPsec(Security Architecture for Internet Protoco)  
IPパケット単位でデータを暗号化する通信プロトコル。トランスポート層やアプリケーション層が暗号化に対応していなくても  
IPsecを利用することで暗号化を利用できる。

### トランスポート層

トランスポート層では、扱うデータの単位をセグメントと呼ぶ。トランポート層の役割はデータ送受信に関しての信頼性の定義をすること。
...
アナログ信号に対して一定周期でデジタル信号をどのくらいの密度でサンプルするかについて標本化定理では、アナログ信号(音声)が表す最大周波数の
2倍の周波数でサンプリングすると、アナログ信号をデジタル信号で再現できるとのこと。

[PCMについての分かりやすい記事](read://https_www.infraexpert.com/?url=https%3A%2F%2Fwww.infraexpert.com%2Fstudy%2Ftelephony2.html)を参照する。
...
[CRCについて分かりやすい記事](https://www.seplus.jp/dokushuzemi/ec/fe/fenavi/mastering_tech/crc/)があったので参照する。

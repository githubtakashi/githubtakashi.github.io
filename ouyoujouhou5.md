...
.  
.  
k回目：上記同様。探したい値の有無に関わらず、整列データが1以上2未満になったら探索は終了する。



元のデータ数をn個とすると、  
要素数は比較回数ごとに半分に減っていく。平均比較回数をkとすると、k回目の要素数は、n/2^kと表せる。

たとえばn＝2の時、平均比較回数k＝1とすると、平均比較回数1のときは、探索が終了した時点での要素数は2/2^1 = 1となる。  

探索対象のデータ数n＝2で、具体的なデータは3、4で、3を探したいとき、

1回目：３を中央値として3と4に2分割→要素は3だけの1個となる（要素数は2/2^1 = 1） 3=3で探索終了。

最大比較回数は、３を探したいときに中央値を4とした場合。4と3で比較して見つからない→要素数1で探索終了。
